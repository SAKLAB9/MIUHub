...
// Supabase Storage 이미지 URL 조회 API (단일 - GET)
app.get('/api/supabase-image-url', async (req, res) => {
  try {
    const { filename } = req.query;
    console.log('[MIUHub API] 단일 이미지 URL 요청 받음, filename:', filename);
    ...
    if (urlError) {
      console.error('[MIUHub API] URL 생성 오류:', urlError);
      return res.status(500).json({ 
        success: false,
        error: 'URL 생성 실패', 
    ...
    console.error('[MIUHub API] 단일 이미지 URL 조회 오류:', error);
    return res.status(500).json({ 
      success: false,
      error: '서버 오류가 발생했습니다.', 
...
// Supabase Storage 이미지 URL 배치 조회 API (POST)
app.post('/api/supabase-image-url', async (req, res) => {
  try {
    const { filenames } = req.body;
    console.log('[MIUHub API] 배치 이미지 URL 요청 받음, filenames:', filenames);
    ...
    console.error('[MIUHub API] 배치 이미지 URL 조회 오류:', error);
    return res.status(500).json({ 
      success: false,
      error: '서버 오류가 발생했습니다.', 
      message: error.message 
